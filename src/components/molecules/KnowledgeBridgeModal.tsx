...
import { KnowledgeBridgeService } from '../../core/knowledge/KnowledgeBridgeService';
import { 
  IKnowledgeBridgeItem, 
  KnowledgeBridgeTypeFilter, 
  IKnowledgeBridgeSearchResult 
} from '../../core/knowledge/KnowledgeBridgeModel';
...
export const KnowledgeBridgeModal: React.FC<KnowledgeBridgeModalProps> = ({
  ...
  // Execute knowledge search whenever query, type filter, or selected tag changes
  useEffect(() => {
    if (!isOpen) return;
    ...
    async function executeSearch() {
      setIsLoading(true);
      const res = await KnowledgeBridgeService.searchKnowledge({
        query: searchTerm,
        typeFilter,
        tag: selectedTag,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm select-none animate-fade-in">
      ...
      >
        ...
        >
          ...
          {/* Type Filter Buttons (Tümü / Sadece Vurgular / Sadece Notlar) */}
        ...
        <div className="p-5 overflow-y-auto flex-1 flex flex-col gap-3">
          {isLoading ? (
            ...
            searchResult.items.map((item) => (
              ...
              >
                {/* Header Row: Type Badge + Page Badge + Date */}
                ...
                {/* Title (if present) & Content Preview */}
                <div className="flex flex-col gap-1">
                  {item.title && (
                ...
                {/* Footer Meta Row: Material Title, Author, Tags, Jump Action */}
