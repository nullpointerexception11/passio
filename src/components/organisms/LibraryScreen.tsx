...
import { SAMPLE_PDF_DOCUMENTS, ISamplePdfDoc, createDemoPdfBuffer } from '../../data/samplePdfs';
import { PdfReaderScreen } from './PdfReaderScreen';
import { PdfEngine } from '../../core/pdf/PdfService';
import { Logger } from '../../core/logger/Logger';
import { useTheme } from '../../core/theme/ThemeContext';
import { KnowledgeBridgeModal } from '../molecules/KnowledgeBridgeModal';
...
export const LibraryScreen: React.FC = () => {
  ...
  // Handle direct navigation state from Writing Editor ("Kaynağa Git")
  useEffect(() => {
  ...
  // Load saved last read page info for sample books
  useEffect(() => {
  ...
  // Open Custom Local PDF File uploaded by user from computer
  ...
  // Selection Navigation from Knowledge Bridge Modal
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-8 flex flex-col gap-8 animate-fade-in">
        {/* User Custom Loaded PDFs Section */}
        {customPdfs.length > 0 && (
        ...
        {/* Curated Sample Books Collection */}
        <section className="flex flex-col gap-4">
          <h2 className="text-xs font-mono font-semibold uppercase tracking-wider text-accent flex items-center gap-2">
      ...
      {/* Render Fullscreen PDF Reader overlay when a session is active */}
      {activePdfSession && (
        <PdfReaderScreen
