...
class SecurityService implements ISecurityService {
  ...
  async isSetupCompleted(): Promise<boolean> {
    ...
    try {
      // Query the database schema 'settings' table built in Sprint 1
  ...
  async setupPin(pin: string): Promise<boolean> {
    ...
    try {
      ...
      // 2. Hash PIN with unique salt
      const pinHash = await this.hashPinWithSalt(pin, salt);

      // 3. Persist the credentials securely inside our offline database settings table
  ...
  async verifyPin(pin: string): Promise<boolean> {
    ...
    try {
      // 1. Retrieve salt and hash from local database
      ...
      // 2. Recalculate target hash
      const computedHash = await this.hashPinWithSalt(pin, salt);

      // 3. Compare hashes (timing-safe simulation)
  ...
  /**
   * Generates a 16-character hexadecimal secure salt
   */
  private generateRandomSalt(): string {
    ...
    if (typeof window !== 'undefined' && window.crypto) {
      ...
      // Fallback node context or math random
      for (let i = 0; i < 8; i++) {
  ...
  /**
   * Cryptographically hashes a PIN string using a unique salt
   */
  private async hashPinWithSalt(pin: string, salt: string): Promise<string> {
    ...
    if (typeof window !== 'undefined' && window.crypto && window.crypto.subtle) {
      ...
      // Simple fast hashing fallback for tests/environments where crypto.subtle is not accessible
  ...
  /**
   * Basic timing-attack-safe comparison logic for hashes
   */
  ...
  /**
   * Fast offline visual fallback hash (for environments lacking Web Crypto Subtle)
   */
  ...
  /**
   * Helper utility to wipe/reset the PIN state. Useful for diagnostic troubleshooting.
   */
